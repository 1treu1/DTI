...
class Interaction_Module(nn.Sequential):
    '''
    ...
    def __init__(self,**config):
        ...
        self.molTokenizer = RobertaTokenizer.from_pretrained(Chemberta_PATH) #Selfberta Tokenizer
        self.molEncoder = RobertaModel.from_pretrained(Chemberta_PATH) #Selfberta Model
        #Paccman_PATH = '/content/drive/MyDrive/DTI/pretrained_roberta/paccman'
        #PaccLarge_PATH = '/content/drive/MyDrive/DTI/pretrained_roberta/exp4_longformer'
        #self.proEncoder = RobertaModel.from_pretrained(PaccLarge_PATH)  #Fastberta Model
        #self.proTokenizer = RobertaTokenizer.from_pretrained(PaccLarge_PATH) #Fastberta Tokenizer
        ...
        self.flatten_dim =  config['flat_dim'] #78192
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=545, nhead=5)
    ...
    def forward(self, d, p, p_mask):
        ...
        inputM = self.molTokenizer(d ,return_tensors = "pt",padding=True)#, truncation=True, max_length=512)
        #print(d)
        #print(len(d))
        inputM['input_ids']=inputM['input_ids'].cuda()
        #print(inputM['input_ids'].shape)
        inputM['attention_mask']=inputM['attention_mask'].cuda()
        #print('here')
        outputM = self.molEncoder(**inputM).pooler_output.half().cuda()
        ...
        d_aug = d_aug.view(self.batch_size,1,768)
        
        d_aug = torch.unsqueeze(d_aug, 2).repeat(1, 1, self.max_p, 1) # repeat along protein size
        #print(d_aug.shape)
        #p_aug = torch.unsqueeze(p_encoded_layers, 1).repeat(1, self.max_d, 1, 1) # repeat along drug size
        p_aug = torch.unsqueeze(p_encoded_layers, 1).repeat(1, 1, 1, 1) # repeat along drug size
        #print(p_aug.shape)
        i = d_aug * p_aug # interaction

        #print('i',i.shape)
        #i_v = i.view(int(self.batch_size/self.gpus), -1, self.max_d, self.max_p) 
        i_v = i.view(int(self.batch_size/self.gpus), -1, 1, self.max_p) 
        # batch_size x embed size x max_drug_seq_len x max_protein_seq_len
        i_v = torch.sum(i_v, dim = 1)
        #print(i_v.shape)
        
        #src = torch.clone(DPint).detach().cuda()
        #print(self.transformer_encoder(src).shape)
        #print(src)
        #print(DPint)
        
        out = self.transformer_encoder(i_v.float().cuda())
        #I = torch.unsqueeze(I,1).repeat(1,1,1,1)
        #print(I.shape)
        #out = self.icnn(i.float().cuda())
        #print('out',out.shape)
        #print(out.shape)
